...
def main(parser):
  ...
  outh1 = open('%s.1.excluded.%s' % (seqprefix,seqtype),'w')
  outh2 = open('%s.2.excluded.%s' % (seqprefix,seqtype),'w')
  for s1,s2 in piter:
    nreads += 1
    readname = s1[0][1:]
    ...
    if args.concordant:
      ''' Both mates must have hit to be excluded '''
      if r1hit and r2hit:
        nremove += 1        
        continue # skip over read
      else:      # write the read
        print >>outh1, '\n'.join(s1)
        print >>outh2, '\n'.join(s2)
    else:
      ''' At least one mate must have hit to be excluded '''
      if r1hit or r2hit:
        nremove += 1        
        continue # skip over read
      else:      # write the read  
  ...
  print >>sys.stderr, '%s%s' % ('Pairs removed:'.ljust(35), nremove)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--concordant', action='store_true', help="Require that both reads in a pair have a hit in order to be removed. Otherwise, read pair will be output if either read has a hit")  
